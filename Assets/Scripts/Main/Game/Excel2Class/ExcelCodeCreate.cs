...
namespace Main.Game.Excel2Class
{
    public class ExcelCodeCreater
    {
        ...
        public static string CreateCodeStrByExcelData(ExcelMediumData excelMediumData)
        {
            ...
            classSource.AppendLine("using HotFix.Common;");
            classSource.AppendLine("using Main.Game.Excel2Class;");
        ...
        private static string CreateExcelAllDataClass(string dataClassName,string itemClassName,ExcelMediumData excelMediumData)
        {
            ...
            classSource.AppendLine();
            //字段初始化方法
            classSource.AppendLine("\tpublic override void Init()");
            classSource.AppendLine("\t{");
            classSource.AppendLine("\t\tbase.Init();");
            ...
            classSource.AppendLine();
            //字典获取方法
            classSource.AppendLine($"\tpublic override ExcelItemBase GetExcelItem({idTypeStr} id)");//{itemClassName}
            classSource.AppendLine("\t{");
        ...
        private static string CreateCodePropertyMethod(string itemClassName,string idTypeStr,string propertyName,string propertyType)
        {
            ...
            methodBuilder.AppendLine($"\t\tvar item = GetExcelItem(id) as {itemClassName};");
            ...
            if(propertyType.Contains("[]"))
            {
                ...
                methodBuilder.AppendLine("\t{");
                methodBuilder.AppendLine($"\t\tvar item0 = GetExcelItem(id) as {itemClassName};");
